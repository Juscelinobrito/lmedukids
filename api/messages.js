...
module.exports = async (req, res) => {
  ...
  if (!API_KEY) {
    return res.status(401).json({
      error: 'OPENAI_API_KEY não configurada. Configure nas variáveis de ambiente da Vercel.'
    });
  }
  ...
  try {
    ...
    if (!imageBase64 || !imageType || !grade) {
      return res.status(400).json({ error: 'Campos obrigatórios: imageBase64, imageType, grade' });
    }
...
IMPORTANTE: Adapte tudo para o ${grade}º ano. Use linguagem simples, animada e encorajadora. Crie exercícios COMPLETAMENTE DIFERENTES da tarefa original (novos valores, novos contextos, mas mesmo assunto). Retorne apenas JSON válido.`;
    ...
    return res.json({ content: clean });

  } catch (err) {
    console.error('Erro ao conectar com a OpenAI:', err.message);
    return res.status(502).json({ error: 'Erro ao conectar com a API OpenAI: ' + err.message });
  }
};
